...
class Emailchef_Drivers_Forms_Elementor extends Emailchef_Drivers_Forms_Abstract
{
    ...
    {
        if ( preg_match( '/^page_(\d+)_form_(.+)$/', $id, $matches ) ) {
            $post_id = $matches[1]; // Estrae l'ID della pagina
            $form_id = $matches[2]; // Estrae l'ID del form
    ...
    public function intercepted($record, $handler)
    {
        ...
        $this->sendSubmission('page_'.$post_id."_form_".$form_id, $data);
